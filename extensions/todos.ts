...
function normalizeTodoSettings(raw: Partial<TodoSettings>): TodoSettings {
	...
	const gcDays = Number.isFinite(raw.gcDays) ? (raw.gcDays as number) : DEFAULT_TODO_SETTINGS.gcDays;
	return {
		gc: Boolean(gc),
	...
	pi.registerTool({
		...
		renderCall(args, theme) {
			...
			if (details.error) {
				...
				const todoDetails = details as Exclude<TodoToolDetails, { action: "list" | "list-all" }>;
				if (!todoDetails.todo) {
					const text = result.content[0];
					return new Text(text?.type === "text" ? text.text : "", 0, 0);
				}

				let text = renderTodoDetail(theme, todoDetails.todo, expanded);
				const actionLabel =
