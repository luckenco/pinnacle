...
import { SimpleFrontmatterCodec } from "./parser";
import { MinimalFrontmatterPatcher } from "./patcher";
import { getDuplicateFrontmatterKeys } from "./validation";
...
describe("MinimalFrontmatterPatcher", () => {
  ...
  it("collapses duplicated disable-model-invocation keys when setting manual-only", () => {
    ...
    assert.ok(
      patch.newText.includes("disable-model-invocation: true\nargument-hint: What next?\n---"),
    );
  });
