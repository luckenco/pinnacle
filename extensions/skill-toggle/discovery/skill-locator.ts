...
import type { LocatedSkillFile, SkillSource } from "../types";
import { getSkillRoots } from "./pi-paths";
  ...
  async findSkillFiles(cwd: string): Promise<LocatedSkillFile[]> {
    ...
    for (const root of roots) {
      ...
      for (const file of found) {
        const canonicalPath = await this.fs.realpath(file.filePath);
        const canonicalFile = { ...file, filePath: canonicalPath };
        const existingIndex = seen.get(canonicalPath);
        if (existingIndex === undefined) {
          seen.set(canonicalPath, files.length);
          files.push(canonicalFile);
          continue;
        }

        if (file.filePath === canonicalPath) {
          files[existingIndex] = canonicalFile;
        }
      }
    }
