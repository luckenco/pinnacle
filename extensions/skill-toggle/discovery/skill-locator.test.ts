...
import { DefaultSkillLocator } from "./skill-locator";
...
describe("DefaultSkillLocator", () => {
  ...
  it("deduplicates symlinked skills by their real path", async () => {
    const target = "/home/tester/.agents/skills/shared/SKILL.md";
    const link = "/home/tester/.pi/agent/skills/shared/SKILL.md";
    const fs = new MemoryTreeFileSystem([link, target], new Map([[link, target]]));
    const locator = new DefaultSkillLocator(fs);

    const files = await locator.findSkillFiles("/repo");

    assert.equal(files.length, 1);
    assert.equal(files[0]?.filePath, target);
  ...
  it("finds global, user, and project skills with Pi root markdown discovery rules", async () => {
    ...
    assert.equal(
      byPath.get("/home/tester/.agents/skills/global-skill/SKILL.md")?.source.kind,
      "global",
    );
